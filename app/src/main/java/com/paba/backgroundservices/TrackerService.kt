    ...
    override fun onDestroy(){
        ...
        try {
            ActivityRecognition.getClient(this).removeActivityTransitionUpdates(pendingIntent)
                .addOnSuccessListener { Log.d("Trackerservist", "Activity updates removes.")}
                .addOnFailureListener { e -> Log.e("TrackerService", "Failed to remove activity updates.", e)}
            unregisterReceiver(activityReceiver)
            sensorManager.unregisterListener(this)
        } catch (e: Exception) {
            Log.e("TrackerService", "Error during unregistering receiver/listeners: ${e.message}")
        }
    }
